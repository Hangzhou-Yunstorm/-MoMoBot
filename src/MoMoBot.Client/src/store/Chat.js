...
const anonymousMessage = messageService.createMomoMessage('不好意思，小摩AI未能认出您，所以不能为您服务。');
...
function requestAnswer(question, ddUserId) {
    return axios({
        ...
        headers: {
            ...
            'x-yc-conversationid': conversationId || ''
        }
    });
}
...
export const actionCreators = {
                ...
                .then(response => {
                    ...
                    if (data.success) {
                        ...
                        // }
                            ...
                            // request({
                            //     url: `${config.serverUrl}/api/voice/tts/${data.message}`,
                            //     method: 'get'
                            // }).then(response => {
                            //     const ttsurl = `${config.serverUrl}/${response}`;
                            let feedback = true;
                            const { data: conData = {} } = data;
                            console.log(conData);
                            if (conData.end) {
                                conversationId = '';
                            } else {

                                conversationId = conData.conversationId || '';
                                if (conversationId !== '') {
                                    feedback = false;
                                }
                            }
                            ...
                            const msg = messageService.createMomoMessage(data.message, false, feedback, feedbackData);
    ...
    saveDDUserInfo: (userInfo) => async (dispatch) => {
        ...
        const { name } = userInfo;
        const userInfoMessage = messageService.createMomoMessage(`${name}你好，我是小摩AI，很高兴为您服务！`);
    ...
    loadingVoice: (id) => async (dispatch) => {
        dispatch({ type: loadVoice, payload: { id, loading: true } });
    },
    loadedVoice: (id) => async (dispatch) => {
        dispatch({ type: loadVoice, payload: { id, loading: false } });
    }
};
