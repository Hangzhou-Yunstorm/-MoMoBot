...
const applicationService = {
    travelApplication: (answer, endCallback) => {
        ...
        if (currentApplicationIndex === TravelApplication.length) {
            ...
            // todo ：发送申请
            return {
                name: '小摩AI', id: guid(), content: <div>您的出差申请内容如下：<ApplicationResultTable data={applicationDataClone} onSubmit={submitApplicationData} /></div>, type: MessageTypes.Html, role: MessageRoles.Other, datetime: new Date()
            }
        }
        ...
        currentApplicationIndex++;
        return { name: '小摩AI', id: guid(), content, type: MessageTypes.Html, role: MessageRoles.Other, datetime: new Date() }
    },
}
